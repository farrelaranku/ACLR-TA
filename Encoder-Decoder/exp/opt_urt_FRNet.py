...
class Opt_URT_FRNet(Exp_Basic):
    def __init__(self, args):
        super(Opt_URT_FRNet, self).__init__(args)
                ...
                if self.args.use_amp:
                    ...
                    if 'B6iFast' in self.args.model:
                        for idx in range(0,self.args.n_learner):
                            outputs = self.model.forward_1learner(batch_x)
                    ...
                    with torch.cuda.amp.autocast():
                        ...
                        if self.args.output_attention:
                            for idx in range(0,self.args.n_learner):
                                ...
                                elif 'B6iFast' in self.args.model:
                                    outputs = self.model.forward_1learner(batch_x)
                                else:
                                    outputs = self.model.forward_1learner(batch_x, batch_x_mark, dec_inp, batch_y_mark,idx=idx)[0]
                                dec_out.append(outputs)
                    ...
                    if self.args.output_attention:
                        for idx in range(0,self.args.n_learner):
                            if self.args.use_multi_gpu and self.args.use_gpu:
                                ...
                                outputs = self.model.forward_1learner(batch_x)[0]
                            else:
                                outputs = self.model.forward_1learner(batch_x, batch_x_mark, dec_inp, batch_y_mark,idx=idx)[0]
                            dec_out.append(outputs)
